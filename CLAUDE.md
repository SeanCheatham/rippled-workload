...
`invariants.py` (`/check/invariants`, driven by `anytime_check_ledger_invariants.sh`) re-queries **xrpld's** validated ledger and fires structural `always` invariants the per-tx assertions can't see — the category ported selectively from the antiralph-ripple harness. Checks: `xrp_balance_non_negative` (sampled tracked accounts) and offer-book structure (`offer_has_required_fields`/`offer_amounts_positive`/`offers_sorted_by_quality`) over the books `offer_create` rests offers in (`_candidate_books` derives them from non-MPT `w.amms` pairs, deduped, both directions). Every check skips silently on RPC failure/`actNotFound` — absence of evidence ≠ violation. Reads xrpld only (cross-validator + XRP supply = sidecar). Offer invariants are must_hit=False (a book may stay empty a whole run); `xrp_balance_non_negative` is must_hit=True. Base-reserve and balance-within-limit are **not** ported (legitimately violable → false positives). Adding an invariant: define the message constant + register it in `assertions.register_assertions`, fire via `assert_invariant`, update this note.
